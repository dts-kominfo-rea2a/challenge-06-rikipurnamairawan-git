...
const bacaData = (fnCallback) => {
  ...
  fs.readFile(file1, 'utf-8', (err, data) => {
    ...
    if (!dataJson1.message.split(" ")[1]) return fnCallback("Data tidak sesuai", null);

    fs.readFile(file2, 'utf-8', (err, data) => {

      const dataJson2 = JSON.parse(data);
      if (!dataJson2[0].message.split(" ")[1]) return fnCallback("Data tidak sesuai", null);

      fs.readFile(file3, 'utf-8', (err, data) => {

        const dataJson3 = JSON.parse(data);
        if (!dataJson3[0].data.message.split(" ")[1]) return fnCallback("Data tidak sesuai", null);
